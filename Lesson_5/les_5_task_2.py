...
class HexNum:
    ...
    def __init__(self, num):
        try:
            ...
            for el in num:
                self.num.append(el.upper())
                if el.upper() not in self.h_table:
                    raise HexError(el)
        except TypeError:
            print('HexNum принимает только итерируемые объекты')
    ...
    def __mul__(self, other):
        ...
        for g in range(len(a) - 1, -1, -1):
            ...
            for j in range(len(b) - 1, -1, -1):
                temp_res = self.h_table[a[g]] * self.h_table[b[j]] + temp
                if temp_res > 15:
                    temp = temp_res // 16
                else:
                    temp = 0
